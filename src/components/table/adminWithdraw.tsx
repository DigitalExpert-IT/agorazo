...
import { useTransactions, TransactionProvider } from "context";
...
export const AdminWithdraw = () => {
  const { withdrawals, withdraw, fetchById, loading, error } = useTransactions();
        ...
        cell: (info) => {
          ...
          return (
            <span
              ...
              className="inline-flex items-center rounded-md px-2 py-1 text-xs font-medium text-white ring-1 ring-inset ring-gray-500/10"
            >
              {info.getValue()}
            </span>
          );
  ...
  return (
    <div className="p-5 space-y-4 mt-20">
      ...
      {modalOpen && withdraw && (
        <Modal onClose={() => setModalOpen(false)} transaction={withdraw} />
      )}
    </div>
  );
};
